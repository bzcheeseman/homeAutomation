...
int prediction(svm_t *params, vector_t *testVector) {
  double fx = params->kernelFunction(params->w, testVector) + params->b;
  if (fx > 1e-5){
    return 1;
  }
  else if (fx < -1e-5){
...
svm_t *coordDescent(vector_t **data, double *y, long lenData, double tol, double C,
  ...
  out->kernelFunction = kernelFunction;
  out->w = newVector(3, NULL);
//  out->b = 0;
  ...
  while (done == 0 && passes < max_passes){
    for (int i = 0; i < lenData; i++){
      double dat[3] = {data[i]->data[0], data[i]->data[1], 1.0};
      vector_t *copyData = newVector(3, dat);
      copyData = transformFunction(copyData);

      double g = y[i]*kernelFunction(copyData, out->w) - 1.0;
      double alphaprime = min( max( out->alphas->data[i] - g/(y[i]*y[i]*kernelFunction(copyData, copyData)), 0 ), C );

      scalarMultiply(copyData, y[i]*(alphaprime - out->alphas->data[i]));
      addVector(out->w, copyData);

      out->alphas->data[i] = alphaprime;


//      double g = y[i]*kernelFunction(data[i], out->w) - 1.0;
//
//      double alphaprime = min( max( out->alphas->data[i] - g/(y[i]*y[i]*kernelFunction(data[i], data[i])), 0 ), C );
//      vector_t *dataCopy = copyVector(data[i]);
//      scalarMultiply(dataCopy, y[i]*(alphaprime - out->alphas->data[i]));
//      addVector(out->w, dataCopy);
//
//      out->alphas->data[i] = alphaprime;
    }

    for (int i = 0; i < lenData; i++){
      if (out->alphas->data[i] > tol){
        done = 0;
      }
      else if (out->alphas->data[i] < tol && i != lenData-1){
        done = 1;
      }
      else if (done == 1 && i == lenData-1){
        ;
      }
    }
    passes++;
  ...
  double outdata[2] = {out->w->data[0], out->w->data[1]};
  out->b = out->w->data[2];
  out->w = newVector(2, outdata);
