...
public:
  message(const std::string &recipient, const std::string &sender) : recipient(recipient), sender(sender) {}
  ...
  inline void remove(int which){ auto iter_which = contents.begin() + which; contents.erase(iter_which); }
  ...
  friend bool serialize(message &message, std::ostream &out){ dlib::serialize(message.contents, out); } //maybe return a zmq message?
